...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const token = jwt.sign(
      { email: result[0].email, isVerified: result[0].isVerified, userId : result[0].userId },
      process.env.NEXTAUTH_SECRET!,
      { expiresIn: "1h" },
    );
    return res.status(200).json({ message: "Login successful", token,user_id:result[0].userId});
  } catch (error) {
